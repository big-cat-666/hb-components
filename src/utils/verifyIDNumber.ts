...
export function checkIDNumber(val: string): boolean {
  if (checkCode(val)) {
    const date = val.slice(6, 14)
    if (checkDate(date)) {
      if (checkProv(val.slice(0, 2))) {
        return true
      }
    }
  }
